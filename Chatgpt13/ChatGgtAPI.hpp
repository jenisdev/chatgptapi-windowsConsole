...
class CHATGPT_API
{
	...
	boost::optional<CHATGPT_RESULT> Text(const char* prompt, int Temperature = 0, int max_tokens = 100)
	{
		...
		auto hi = r.RequestWithBuffer(L"/v1/chat/completions", L"POST", hdrs, data.data(), data.size());
		std::vector<char> out;
		...
		try
		{
			...
			if (o.has<jsonxx::Object>("error"))
				...
				auto& message = choice0.get<jsonxx::Object>("message");
				r.t = message.get<jsonxx::String>("content");
			}
			return r;
		}
		catch (...)
